...
def create_server_app(tomebox):
            ...
            if client_token == server_token:
                ...
                try:
                    ...
                    if hashed_client in paired_devices:
                        ...
                        now = time.time()
                        if (
                            now - paired_devices[hashed_client].get("last_seen", 0)
                            > 3600
                        ):
    ...
    def refresh_library():
        ...
        try:
            ...
            if hasattr(tomebox, "logger"):
                tomebox.logger(f"API Error - Refresh failed: {e}")
            raise HTTPException(
                status_code=500, detail="Internal server error during library refresh."
            )
    ...
    def authenticate_device(request: Request, token: str = None, otp: str = None):
        ...
        return HTMLResponse(
            "<h1>Expired Link</h1><p>This pairing code has expired. Please generate a new one from the TomeBox desktop app.</p>",
            status_code=401,
        )
    ...
    def web_interface():
        ...
        try:
            ...
            return HTMLResponse(
                content="<h1>Error: mobile_ui.html not found</h1>", status_code=404
            )
    ...
    def desktop_landing(request: Request):
        ...
        try:
            ...
            return HTMLResponse(
                content="<h1>Error: desktop_ui.html not found</h1>", status_code=404
            )
    ...
    async def add_to_shelf(request: Request):
        ...
        if not asin or not shelf_name:
            raise HTTPException(
                status_code=400, detail="ASIN and shelf name are required."
            )
    ...
    def get_web_library():
        ...
        if not hasattr(tomebox, "_web_master_metadata"):
            ...
            if os.path.exists(data_dir):
                for f in os.listdir(data_dir):
                    if (
                        f.startswith("cloud_") and f.endswith(".json")
                    ) or f == "cloud_cache.json":
                        try:
                            with open(os.path.join(data_dir, f), "r") as file:
                                for item in json.load(file):
                                    if item.get("asin"):
                                        master_metadata[item["asin"]] = item
                                    if item.get("title"):
        ...
        for path, data in tomebox.library_manager.local_library.items():
            if data.get("asin"):
                master_metadata[data["asin"]] = data
            if data.get("title"):
                master_metadata[data["title"]] = data
        ...
        for path, data in tomebox.library_manager.local_library.items():
            ...
            if isinstance(existing_auth, list):
                item_copy["authors"] = ", ".join(
                    [
                        a.get("name", "") if isinstance(a, dict) else str(a)
                        for a in existing_auth
                    ]
                )

            meta = master_metadata.get(asin) or master_metadata.get(
                item_copy.get("title"), {}
            )
            if meta:
                if not item_copy.get("authors") or item_copy.get("authors") in [
                    "Unknown",
                    "Unknown Author",
                ]:
        ...
        local_titles = {
            ...
            for data in tomebox.library_manager.local_library.values()
        }
        local_asins = {
            data.get("asin")
            for data in tomebox.library_manager.local_library.values()
                ...
                if getattr(tomebox, "file_path", None) == path:
                    if getattr(tomebox, "root", None):
                        tomebox.root.after(
                            0,
                            lambda: (
                                tomebox.playback_presenter.sync_playhead_from_remote(
                                    position
                                )
                            ),
                        )
    ...
    def get_chapters(path: str):
        if not path or path not in tomebox.library_manager.local_library:
            raise HTTPException(
                status_code=403,
                detail="Forbidden: File is not registered in the local library.",
            )
        ...
        if not raw_chapters:
            ...
            try:
                ...
                if hasattr(tomebox, "logger"):
                    tomebox.logger(
                        f"API Error - FFprobe chapter extraction failed: {e}"
                    )
                return []
    ...
    def list_profiles_detailed():
        ...
        active = tomebox.settings.get(
            "active_profile", profiles[0] if profiles else "Main"
        )
    ...
    async def create_profile(request: Request):
        ...
        if not name.replace("_", "").replace("-", "").replace(" ", "").isalnum():
            raise HTTPException(
                status_code=400, detail="Profile name must be alphanumeric"
            )
        ...
        if os.path.exists(auth_path):
            try:
                ...
                try:
                    ...
                    if hasattr(tomebox, "logger"):
                        tomebox.logger(
                            f"Library fetch failed during profile switch: {e}"
                        )
            except Exception as e:
                if hasattr(tomebox, "logger"):
                    tomebox.logger(f"API Error - Failed to load auth for {name}: {e}")
                raise HTTPException(
                    status_code=500, detail="Failed to load authentication for profile."
                )
        else:
    ...
    async def auth_login_start(request: Request):
        ...
        if not state["url_ready"].wait(timeout=15):
            raise HTTPException(
                status_code=500, detail="Failed to generate login URL in time"
            )

        if state.get("error"):
            if hasattr(tomebox, "logger"):
                tomebox.logger(f"API Error - Login start failed: {state['error']}")
            raise HTTPException(
                status_code=500, detail="Login process failed or was cancelled."
            )
    ...
    async def auth_login_complete(request: Request):
        ...
        if not state:
            raise HTTPException(
                status_code=400, detail="No pending login for this profile"
            )
        ...
        if not waited:
            raise HTTPException(
                status_code=504, detail="Amazon token exchange timed out."
            )

        if state.get("error"):
            if hasattr(tomebox, "logger"):
                tomebox.logger(f"API Error - Login complete failed: {state['error']}")
    ...
    def get_download_queue():
        ...
        queue_list = [
            {"asin": task["asin"], "title": task["title"]}
            for task in tomebox.download_manager.queue
        ]

        return {
            "is_processing": tomebox.download_manager.is_processing,
            "active": getattr(
                tomebox.download_manager,
                "web_state",
                {"active_asin": None, "progress": 0, "status": "Idle"},
            ),
            "queue": queue_list,
        }
    ...
    async def queue_downloads(request: Request):
        ...
        if not save_dir:
            ...
            return JSONResponse(
                status_code=400, content={"detail": "DOWNLOAD_DIR_NOT_SET"}
            )
    ...
    async def set_download_dir(request: Request):
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Cannot create or access directory: {e}"
            )
    ...
    def get_conversion_queue():
        return {
            ...
            "is_processing": getattr(
                tomebox.conversion_manager, "current_process", None
            )
            is not None,
        }
    ...
    async def import_local_path(request: Request):
        ...
        try:
            ...
            if not import_root:
                raise HTTPException(
                    status_code=500,
                    detail="Server misconfiguration: No import root defined.",
                )
            ...
            if os.path.commonpath([abs_root, abs_requested]) != abs_root:
                if hasattr(tomebox, "logger"):
                    tomebox.logger(
                        f"SECURITY BLOCKED: Attempted import outside safe boundary -> {path}"
                    )
                raise HTTPException(
                    status_code=403,
                    detail="Forbidden: Path is outside the allowed import directory.",
                )
            ...
            if hasattr(tomebox, "logger"):
                tomebox.logger(f"API Error - Path validation failed: {e}")
            raise HTTPException(
                status_code=500, detail="Internal server error during path validation."
            )
        ...
        try:
            ...
            def status_cb(msg):
                if hasattr(tomebox, "logger"):
                    tomebox.logger(f"[Web Import] {msg}")

            def complete_cb(count, total=0):
                if hasattr(tomebox, "logger"):
                    tomebox.logger(
                        f"[Web Import] Finished adding {count}/{total} files."
                    )
            ...
            if hasattr(tomebox, "logger"):
                tomebox.logger(f"API Error - Local path import failed: {e}")
            raise HTTPException(
                status_code=500, detail="Internal server error during import task."
            )
    ...
    async def match_local_file(request: Request):
        ...
        if not path or path not in tomebox.library_manager.local_library:
            raise HTTPException(
                status_code=403,
                detail="Forbidden: Target file is not registered in the local library.",
            )
        # --------------------------------------------

        if not asin or not os.path.exists(path):
            raise HTTPException(
                status_code=400, detail="Invalid ASIN or Path missing from disk."
            )
    ...
    async def search_audible_catalog(request: Request):
        ...
        try:
            ...
            if hasattr(tomebox, "logger"):
                tomebox.logger(f"Web UI Audible Search Error: {e}")
        ...
        try:
            ...
            if hasattr(tomebox, "logger"):
                tomebox.logger(f"Web UI Google Books Search Error: {e}")
    ...
    async def scrape_metadata(request: Request):
        ...
        try:
            ...
            return {
                "status": "success",
                "message": "Scrape and embed started in background",
            }
        except Exception as e:
            if hasattr(tomebox, "logger"):
                tomebox.logger(f"API Error - Metadata scrape failed: {e}")
            raise HTTPException(
                status_code=500, detail="Internal server error during metadata scrape."
            )
    ...
    def get_bookmarks(path: str):
        if path and path in tomebox.library_manager.local_library:
            return {
                "bookmarks": tomebox.library_manager.local_library[path].get(
                    "bookmarks", []
                )
            }
        return {"bookmarks": []}
