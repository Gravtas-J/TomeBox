...
def test_fetch_cloud_library_success(cloud_controller, mock_app):
    ...
    mock_app.ui_state.dl_status.set.assert_called_with(
        "Fetching data from Amazon... Please wait."
    )
    mock_app.thread_pool.submit.assert_called_once_with(
...
def test_fetch_library_worker_network_error(cloud_controller, mock_app):
    ...
    mock_app.library_manager.fetch_cloud_library.side_effect = httpx.ConnectError(
        "Offline"
    )
