...
def test_api_stream_range_parsing(auth_client, fake_tomebox, tmp_path):
    ...
    res = auth_client.get(
        f"/api/stream?path={path_str}", headers={"Range": "bytes=0-499"}
    )
    ...
    # 3. Beyond EOF Range
    res = auth_client.get(
        f"/api/stream?path={path_str}", headers={"Range": "bytes=2048-4096"}
    )
...
def test_api_bookmarks_crud(auth_client, fake_tomebox):
    ...
    res = auth_client.post(
        ...
        json={"path": path, "time": 120.5, "note": "Great quote"},
    )
    assert res.status_code == 200
    ...
    res = auth_client.request(
        "DELETE", "/api/library/bookmarks", json={"path": path, "index": 0}
    )
    assert res.status_code == 200
...
def test_api_progress_update(auth_client, fake_tomebox):
    ...
    res = auth_client.post(
        "/api/progress", json={"path": path, "position": "145.6", "profile": "Main"}
    )
