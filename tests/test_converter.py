...
def test_resolve_cover_path(monkeypatch):
    ...
    def mock_exists_padded(p):
        return "0000000123.jpg" in str(p)
    ...
    def mock_exists_generic(p):
        return str(p).endswith("folder.jpg")
...
def test_get_metadata_and_chapters(converter, monkeypatch):
    ...
    minimal_output = {"format": {"tags": {"Title": "Minimal"}}}
...
def test_convert_to_m4b_missing_ffmpeg(converter, monkeypatch):
    monkeypatch.setattr(
        "core.converter.ProcessRunner.run_async",
...
def test_split_into_chapters(converter, mock_process, monkeypatch):
    ...
    mock_progress = MagicMock()
    result = converter.split_into_chapters(
        "/in.m4b", "/target", chapters, [], mock_progress
    )
...
def test_split_into_chapters_cancellation(converter, mock_process, monkeypatch):
    ...
    with pytest.raises(Exception, match="Chapter splitting cancelled"):
        converter.split_into_chapters(
            "/in.m4b", "/target", [{"start_time": 0, "end_time": 10}], []
        )
...
def test_concat_to_m4b(converter, mock_process, monkeypatch):
    ...
    mock_meta = MagicMock(
        return_value={"format": {"duration": "100.0", "tags": {"artist": "Artist"}}}
    )
