    ...
    [
        ...
        (
            [
                ...
                {"title": "Series B", "sequence": "2"},
            ],
            "Series A (Bk 1), Series B (Bk 2)",
        ),
...
def test_find_matching_fuzzy():
    ...
    match = find_matching_cloud_item(
        "Dungeon Crawler Carl [Soundtrack included]", cloud_items
    )
    assert match is not None
...
def test_find_matching_series_prefix():
    ...
    match = find_matching_cloud_item(
        "The Expanse Book 1 - Leviathan Wakes", cloud_items
    )
    assert match is not None
...
def test_format_series_list_edge_cases():
    ...
    # Valid single entry
    assert (
        format_series_list([{"title": "The Expanse", "sequence": "1"}])
        == "The Expanse (Bk 1)"
    )
    ...
    assert (
        format_series_list(
            [{"title": "Series A", "sequence": "1"}, {"title": "Series B"}]
        )
...
def test_find_matching_cloud_item_series_aware():
    ...
    match = find_matching_cloud_item(
        "The Wheel of Time The Eye of the World", cloud_items
    )
    assert match["asin"] == "1"
...
def test_find_matching_cloud_item_series_missing_title():
    core.utils.text.RAPIDFUZZ_AVAILABLE = True
    cloud_items = [
        {"title": "Cloud Book", "series": [{"sequence": "1"}]}
    ]  # Missing 'title'
