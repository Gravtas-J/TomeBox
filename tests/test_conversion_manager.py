...
def test_convert_single_flow_and_original_deletion(manager, monkeypatch):
    ...
    manager.library_manager.local_library[input_file] = {
        "title": "Test Book",
        "asin": "123",
    }
...
def test_split_book_preserves_original(manager, monkeypatch):
    ...
    if mock_unlink.called:
        for args_call in mock_unlink.call_args_list:
            assert args_call[0][0] != input_file, (
                "Original file was erroneously deleted during split!"
            )
...
def test_batch_error_accumulation(manager, monkeypatch):
    ...
    )  # Pretend all 5 paths exist on disk
