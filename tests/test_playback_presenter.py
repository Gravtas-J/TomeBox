...
def test_on_playback_tick(mock_time, presenter, mock_app):
    ...
    presenter.on_playback_tick(
        current_time=25.0, total_time=100.0, real_time_delta=60.0
    )
...
def test_cue_last_played(presenter, mock_app):
    mock_app.settings = {"last_played_Main": "/mock/last.m4b"}
...
def test_speed_and_volume_change_while_playing(presenter, mock_app):
    ...
    with (
        patch.object(presenter, "pause_audio") as mock_pause,
        patch.object(presenter, "resume_playback") as mock_resume,
    ):
        # Speed
        mock_app.ui_state.playback_speed.get.return_value = "1.5x"
...
def test_sleep_timer_chapters(presenter, mock_app):
    ...
    mock_app.sleep_mode = "chapters"
    mock_app.sleep_chapters_remaining = 1
