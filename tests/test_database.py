...
def test_settings_json_fallback(temp_db):
    ...
    cursor.execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", ("bad_json_key", "raw_string_value"))
    temp_db.conn.commit()
...
def test_hash_device_token_json_fallback(temp_db):
    ...
    cursor = temp_db.conn.cursor()
    cursor.execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", ("device_salt", "raw_salt"))
    temp_db.conn.commit()
...
def test_local_db_malformed_json_skip(temp_db, monkeypatch):
    ...
    cursor = temp_db.conn.cursor()
    cursor.execute("INSERT OR REPLACE INTO library (path, data) VALUES (?, ?)", ("/fake/corrupt.m4b", "{bad_json"))
    cursor.execute("INSERT OR REPLACE INTO library (path, data) VALUES (?, ?)", ("/fake/good.m4b", '{"title": "Good"}'))
    temp_db.conn.commit()
