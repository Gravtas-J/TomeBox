...
import sys
from api.audible_client import AudibleClient, RateLimitError, APIUnavailableError, find_key_iv_in_voucher, find_url_in_response
...
def test_auth_file_operations(client, monkeypatch):
    ...
    monkeypatch.setattr("api.audible_client.audible.Authenticator.from_file", mock_authenticator)
...
def test_get_download_license_offline_key(client, monkeypatch):
    ...
    mock_post.return_value = {
        ...
        "content_license": {
            "content_metadata": {
                "content_key": {"offline_key": "fake_base64_key"}
            }
        }
    }
    ...
    monkeypatch.setattr("rsa.PrivateKey.load_pkcs1", MagicMock(return_value=mock_rsa_key))
    monkeypatch.setattr("rsa.decrypt", MagicMock(return_value=b'1234567890123456abcdefghijklmnop'))
...
def test_find_key_iv_misses():
    ...
    # Dict miss
    assert find_key_iv_in_voucher({"wrong": "data", "nested": {"still": "wrong"}}) == (None, None)
    # List miss
...
def test_login_with_browser(client, monkeypatch):
    ...
    mock_auth = MagicMock()
    monkeypatch.setattr("api.audible_client.audible.Authenticator.from_login_external", MagicMock(return_value=mock_auth))
...
def test_fetch_product_metadata_and_search(client, monkeypatch):
    ...
    mock_request.return_value = {"product": {"title": "Single Book"}, "products": [{"title": "Search Book"}]}
    monkeypatch.setattr(client, "_request_with_backoff", mock_request)
    monkeypatch.setattr("api.audible_client.audible.Client", MagicMock())
...
def test_get_download_license_voucher_fallback(client, monkeypatch):
    ...
    client.auth = True
    mock_client = MagicMock()
    mock_client.return_value.post.return_value = {"offline_url": "http://test", "content_license": {}}
    monkeypatch.setattr("api.audible_client.audible.Client", mock_client)
    
    # Mock the voucher decryption tool
    mock_decrypt = MagicMock(return_value={"key": "v_key", "iv": "v_iv"})
    monkeypatch.setattr("api.audible_client.decrypt_voucher_from_licenserequest", mock_decrypt)
...
def test_get_drm_flags(client, monkeypatch):
    ...
    flags = client.get_drm_flags("book.aaxc", {"audible_key": "k1", "audible_iv": "i1"}, "Main", "deadbeef", data_dir)
    assert flags == ["-audible_key", "k1", "-audible_iv", "i1"]
    
    # Case 2: File belongs to the active profile, use the globally provided auth_bytes
    flags = client.get_drm_flags("book.aax", {"owner": "Main"}, "Main", "deadbeef", data_dir)
    ...
    monkeypatch.setattr("api.audible_client.audible.Authenticator.from_file", MagicMock(return_value=mock_temp_auth))
    
    flags = client.get_drm_flags("book.aax", {"owner": "Wife"}, "Main", "deadbeef", data_dir)
    ...
    flags = client.get_drm_flags("book.aax", {"owner": "Wife"}, "Main", "deadbeef", data_dir, logger=mock_logger)
