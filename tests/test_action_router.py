...
def test_download_progress_event_updates_global_and_queue(router, mock_app):
    ...
    router.event_bus.publish("download.progress", asin="123", percent=50.0, is_global=True)
...
def test_conversion_complete_event(router, mock_app, monkeypatch):
    ...
    messagebox_mock.showinfo.assert_called_with("Conversion Success", "Merge successful")

def test_task_error_event(router, mock_app):
    # Setup the failed tasks list as empty
    mock_app.failed_tasks = []
    
    router.event_bus.publish("conversion.error", filepath="book.mp3", action="split", error_msg="FFmpeg failed")
...
def test_import_status_and_progress(router, mock_app):
    ...
    prog_mock = MagicMock()
    mock_app.queue_ui_elements["import_999"] = {"status_var": status_mock, "prog_var": prog_mock}
...
def test_import_finished_success(router, mock_app):
    ...
    status_mock = MagicMock()
    mock_app.queue_ui_elements["import_777"] = {"frame": MagicMock(), "status_var": status_mock}
    
    router.on_import_finished("C:/fake/path", added_count=2, total_found=2, task_id="import_777")
    
    mock_app.system_manager.remove_pending_import.assert_called_with(mock_app.db.data_dir, "C:/fake/path")
...
def test_book_start_and_complete(router, mock_app):
    ...
    status_mock = MagicMock()
    mock_app.queue_ui_elements["sub_task_1"] = {"frame": MagicMock(), "status_var": status_mock}
...
def test_scrape_error_handling(router, mock_app):
    ...
    mock_app.ui_state.dl_status.set.assert_called_with("Audible API rate limit reached. Pausing scrape for 60s.")
