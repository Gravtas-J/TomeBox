...
def test_process_single_file_extracts_embedded_tags(
    manager, mock_converter, monkeypatch
):
    """Validates fallback to FFprobe tags and local hashing when no cloud match exists."""

    # Force cloud match to fail
    monkeypatch.setattr(
        "core.controllers.library_manager.find_matching_cloud_item",
...
def test_process_single_file_applies_cloud_match(manager, mock_converter, monkeypatch):
    ...
    monkeypatch.setattr(
        "core.controllers.library_manager.find_matching_cloud_item",
        lambda *args, **kwargs: mock_match,
    )

    entry = manager._process_single_file_for_import(
        filepath="/fake/book.mp3", active_profile="Main", converter=mock_converter
    )
...
def test_save_playback_state_valid_update(manager):
    ...
    manager.local_library = {filepath: {"title": "Test Book"}}
...
def test_build_master_metadata_from_disk(manager, monkeypatch):
    ...
    monkeypatch.setattr(
        os, "listdir", lambda p: ["cloud_cache.json", "cloud_wife.json", "ignore.txt"]
    )
...
def test_silent_cloud_sync(manager, monkeypatch):
    manager.api.auth = True
    manager.api.fetch_library.return_value = [
        {"asin": "123", "title": "New Silent Book"}
    ]
...
def test_monitor_local_files(manager, monkeypatch):
    ...
    monkeypatch.setattr(
        os.path, "exists", lambda p: p == "/fake/exists.m4b" or "sqlite" in p
    )
    monkeypatch.setattr(
        os.path, "getmtime", lambda p: 5000.0
    )  # Simulate a new external DB write
...
def test_import_files_worker(manager, monkeypatch):
    ...
    manager.import_files(
        ["/fake/1.m4b", "/fake/2.mp3"], MagicMock(), "Main", MagicMock(), mock_complete
    )
...
def test_import_folder_grouping_and_merging(manager, monkeypatch):
    ...
    def mock_walk(path):
        return [
            ...
            (
                ...
                [],
                ["file.aax"],
            ),  # Should import direct (AAX bypass)
        ]
    ...
    manager.import_folder(
        "/fake/folder", mock_converter, "Main", MagicMock(), mock_complete
    )
...
def test_build_playlist_entry_timeline_math(manager, monkeypatch):
    ...
    monkeypatch.setattr(
        "core.controllers.library_manager.ProcessRunner.run_blocking", MagicMock()
    )
    monkeypatch.setattr(os.path, "exists", lambda p: False)
...
def test_monitor_preserves_virtual_playlists(manager, monkeypatch):
    ...
    manager.local_library = {
        ...
        "/fake/deleted_file.m4b": {"title": "Ghost File"},
    }
    ...
    def test_import_folder_playlist_rebuild_preserves_data(manager, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "walk",
            lambda p: [
                ("/fake/folder/MyAudiobook", [], ["track01.mp3", "track02.mp3"])
            ],
        )
...
@patch("os.path.isdir", return_value=True)
@patch("os.walk")
def test_import_folder_file_already_in_library(
    mock_walk, mock_isdir, manager, mock_converter
):
...
def test_import_folder_file_vanished_during_scan(
    mock_exists, mock_walk, mock_isdir, manager, mock_converter
):
...
def test_import_folder_metadata_crash(
    mock_exists, mock_walk, mock_isdir, manager, mock_converter
):
    ...
    manager._process_single_file_for_import = MagicMock(
        side_effect=Exception("FFprobe catastrophic failure")
    )
...
@patch("os.walk")
@patch("os.path.exists")
def test_import_folder_existing_merge_bypasses_concat(
    mock_exists, mock_walk, mock_isdir, manager, mock_converter
):
    ...
    mock_walk.return_value = [
        ("/fake/dir", [], ["01.mp3", "02.mp3", "Embedded Album.m4b"])
    ]
    mock_exists.return_value = True

    manager._process_single_file_for_import = MagicMock(
        return_value={"title": "Merged Book"}
    )

    manager.import_folder(
        "/fake/dir", mock_converter, "Main", None, None, import_mode="merge"
    )
    worker = manager.import_queue.pop(0)["func"]
    worker()
...
@patch("os.walk")
@patch("os.path.exists")
def test_import_folder_merge_failure_fallback(
    mock_exists, mock_walk, mock_isdir, manager, mock_converter
):
...
@patch("os.walk")
@patch("os.path.exists", return_value=True)
def test_import_folder_missing_metadata_fallback(
    mock_exists, mock_walk, mock_isdir, manager, mock_converter
):
    ...
    mock_walk.return_value = [
        ("/fake/My_Custom_Folder", [], ["Track_01.mp3", "Track_02.mp3"])
    ]
    ...
    manager._process_single_file_for_import = MagicMock(
        return_value={"title": "My_Custom_Folder", "authors": "Unknown Author"}
    )

    manager.import_folder(
        "/fake/My_Custom_Folder",
        mock_converter,
        "Main",
        None,
        None,
    ...
    assert (
        manager._process_single_file_for_import.call_count == 0
    )  # Bypassed single import
