...
def mock_api():
    ...
    api = MagicMock()
    api.get_download_license.return_value = ("https://fake.url/download", "fake_key", "fake_iv")
    return api
...
def test_download_success_aax(downloader, monkeypatch):
    ...
    downloader.api.get_download_license.return_value = ("https://fake.url/download", None, None)
...
def test_download_cleanup_os_error(downloader, monkeypatch):
    ...
    import urllib.request
    monkeypatch.setattr(urllib.request, "urlopen", MagicMock(side_effect=Exception("Crash")))
    monkeypatch.setattr(os.path, "exists", lambda p: True)
