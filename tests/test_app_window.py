...
def app_instance(
    ...
    # Prevent the UI components from crashing trying to draw to the headless mock
    with (
        patch("ui.app_window.setup_menu_bar", side_effect=fake_setup_menu),
...
def test_cancel_active_task_global(app_instance):
    ...
    app_instance.action_router.on_dl_status.assert_called_with(
        "import_123", "Canceling...", is_global=False
    )
...
def test_start_convert_all_thread(mock_yesno, app_instance):
    ...
    app_instance.conversion_manager.convert_batch.assert_called_once_with(
        ["/fake/1.aax", "/fake/3.aaxc"]
    )
...
def test_export_csv_worker(mock_exporter, mock_asksave, app_instance):
    ...
    mock_exporter.export_csv.assert_called_with(
        "/mock/export.csv",
        app_instance.library_manager.local_library,
        app_instance.library_manager.cloud_items,
    )
...
def test_export_html_worker(mock_open, mock_exporter, mock_asksave, app_instance):
    ...
    mock_exporter.export_html.assert_called_with(
        "/mock/export.html",
        app_instance.library_manager.local_library,
        app_instance.library_manager.cloud_items,
    )
...
def test_manage_shelves_prompt_success(mock_open_shelf, app_instance):
    app_instance.current_view_mode = "grid"
    app_instance._selected_grid_item = {
        "values": ["The Book", "Auth", "Narrator", "Ser", "Dur", "ASIN123"]
    }
    app_instance.manage_shelves_prompt()
...
def test_on_item_select_list_mode(app_instance):
    ...
    app_instance.library_tree.item.return_value = {
        "values": ["Test Title", "Author Name", "Narrator", "Series", "Dur", "ASIN_X"]
    }

    app_instance.library_manager.local_library = {
        "/fake/file.m4b": {"title": "Test Title"}
    }
    app_instance.author_label = MagicMock()
    app_instance.cover_label = MagicMock()
...
def test_handle_action_download(mock_yesno, app_instance):
    ...
    app_instance.current_view_mode = "grid"
    app_instance._selected_grid_item = {
        "values": ["Cloud Book", "Auth", "Narr", "Ser", "Dur", "ASIN123"]
    }
    ...
    with patch.object(app_instance, "ensure_download_folder", return_value="/mock/dl"):
        ...
        app_instance.import_session.add_queue_ui_row.assert_called_with(
            "ASIN123", "Cloud Book"
        )
        app_instance.download_manager.queue_download.assert_called_with(
            "ASIN123", "Cloud Book", "/mock/dl", post_action="download"
        )
...
def test_handle_action_play_local(mock_exists, app_instance):
    ...
    app_instance.current_view_mode = "grid"
    app_instance._selected_grid_item = {
        "values": ["Local Book", "Auth", "Narr", "Ser", "Dur", "ASIN123"]
    }
    app_instance.library_manager.local_library = {
        "/mock/file.m4b": {"title": "Local Book"}
    }

    app_instance.handle_action_on_selected("play")

    app_instance.playback_presenter.load_specific_file.assert_called_with(
        "/mock/file.m4b"
    )
...
def test_start_convert_thread_split(mock_askdir, app_instance):
    ...
    app_instance.library_manager.local_library = {
        "/mock/file.m4b": {
            "chapters": [
                {"id": 0, "tags": {"title": "Ch 1"}},
                {"id": 1, "tags": {"title": "Ch 2"}},
            ]
        }
    }
