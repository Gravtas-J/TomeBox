...
class AAXManagerApp:
    ...
    def on_ui_mode_change(self, event=None):
        new_mode = self.ui_mode_var.get()
        self.settings["ui_mode"] = new_mode
        self.save_settings()
        
        # Disable the dark mode toggle if classic is selected, as it relies on sv_ttk
        if new_mode == "classic":
    ...
    def build_library_components(self, parent):
        ...
        lib_frame = ttk.LabelFrame(self.main_paned, text="Unified Library", padding=10)
...
if __name__ == "__main__":
    ...
    # Only boot the modern theme if the setting allows it (defaults to modern)
    if app.settings.get("ui_mode", "modern") == "modern":
        import sv_ttk
        sv_ttk.set_theme("dark") 
