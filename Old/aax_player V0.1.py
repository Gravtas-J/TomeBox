...
class AAXManagerApp:
    ...
    def setup_ui(self):
        ...
        self.tab_player = ttk.Frame(notebook)
        self.tab_settings = ttk.Frame(notebook)

        notebook.add(self.tab_library, text="Library")
        notebook.add(self.tab_player, text="Player & Converter")
    ...
    def build_library_tab(self):
        cloud_frame = tk.LabelFrame(self.tab_library, text="Cloud Library (Available on Audible)", padx=10, pady=5)
        cloud_frame.pack(fill="both", expand=True, padx=10, pady=5)

        # Added ASIN to the columns
        ...
        self.cloud_tree.column("Title", width=200)
        self.cloud_tree.column("Author", width=120)
        self.cloud_tree.column("Duration", width=70)
        self.cloud_tree.column("ASIN", width=90)
        ...
        local_frame = tk.LabelFrame(self.tab_library, text="Local Library (Downloaded & Converted)", padx=10, pady=5)
        ...
        self.local_tree.column("Title", width=200)
        self.local_tree.column("Format", width=60)
        self.local_tree.column("Path", width=220)
    ...
    def send_to_player(self):
        ...
        self.root.nametowidget(self.tab_player.winfo_parent()).select(self.tab_player)
        self.load_specific_file(filepath)

    # --- TAB 3: PLAYER & CONVERTER ---
    def build_player_tab(self):
        file_frame = tk.LabelFrame(self.tab_player, text="Active File", padx=10, pady=10)
        ...
        play_frame = tk.LabelFrame(self.tab_player, text="Playback", padx=10, pady=10)
        play_frame.pack(fill="x", padx=10, pady=5)

        self.info_label = tk.Label(play_frame, text="", fg="blue", wraplength=450)
        self.info_label.pack(pady=5)

        # Tracker / Visualizer variables
