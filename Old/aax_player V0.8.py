...
class AAXManagerApp:
    def __init__(self, root):
        ...
        self.cloud_items = [] # NEW: Stores the raw API data
    ...
    def build_library_components(self, parent):
        ...
        self.library_tree = ttk.Treeview(tree_frame, columns=("Title", "Author", "Duration", "ASIN", "Status"), show="headings", yscrollcommand=scroll.set)
        ...
        self.library_tree.column("Author", width=150)
        self.library_tree.column("Duration", width=70)
        self.library_tree.column("ASIN", width=100)
        ...
        tk.Button(btn_frame, text="Play Selected", command=lambda: self.handle_action_on_selected("play")).pack(side=tk.LEFT, padx=5)
    ...
    def refresh_library_ui(self):
        ...
        # Build a lookup dictionary for local files based on Title
        local_titles = {data["title"]: data for path, data in self.local_library.items()}
        cloud_titles = []

        # 1. Add all cloud items and cross-reference download status
        for item in self.cloud_items:
            ...
            self.library_tree.insert("", "end", values=(title, authors, duration_str, asin, status))

        # 2. Add orphaned local files (manually added files not from your Audible cloud)
        for path, data in self.local_library.items():
            if data["title"] not in cloud_titles:
                self.library_tree.insert("", "end", values=(data["title"], "Local File", "N/A", "Unknown", f"Downloaded ({data['format']})"))
    ...
    def fetch_cloud_library(self):
        ...
        # Updated to target library_tree and pad the 5th 'Status' column
    ...
    def fetch_library_worker(self):
        try:
            ...
            # Save the raw data so we can cross-reference it later
            ...
            self.write_log(f"ERROR FETCHING LIBRARY: {e}")
