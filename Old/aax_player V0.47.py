    ...
    def _get_mobile_html(self):
        ...
        <head>
            ...
            <style>
                ...
                #search-box { width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #444; background-color: #222; color: white; box-sizing: border-box; font-size: 1rem; outline: none; }
        ...
        <body>
            ...
            <div id="player-bar">
                ...
                <div class="player-bottom-row">
                    ...
                    <div style="min-width: 50px;"></div> </div>
            ...
            <script>
                ...
                async function loadLibrary() {
                    try {
                        ...
                        for (const [path, data] of Object.entries(library)) {
                            ...
                            // Format resume position for the UI pill
                            let timePill = "";
                            if (resumePos > 60) {
                            ...
                            card.onclick = () => startPlayback(path, titleStr, authorStr, resumePos);
                ...
                function filterLibrary() {
                    ...
                    allBooks.forEach(book => {
                        book.element.style.display = book.searchString.includes(query) ? 'flex' : 'none';
                    });
                }

                async function startPlayback(filePath, title, author, startPosition) {
                    ...
                    // Fetch chapters in the background
                    try {
                    ...
                    if ('mediaSession' in navigator) {
                        navigator.mediaSession.metadata = new MediaMetadata({ title: title, artist: author, album: 'TomeBox' });
                ...
                // --- NEW: Chapter Skip Logic ---
                function skipChapter(direction) {
                    if (!currentChapters.length) {
                        // Fallback to 3-minute skips if no chapters exist
                        skipAudio(direction * 180); 
                        return;
                    }
                    const now = audio.currentTime;
                    if (direction === 1) { 
                        // Find the very next chapter
                        const nextCh = currentChapters.find(c => c.start > now + 2);
                        if (nextCh) audio.currentTime = nextCh.start;
                    } else { 
                        // Find the chapter before the current one (allow 3 seconds of grace period)
                ...
                // --- NEW: Heartbeat Sync Loop ---
                // Pings the backend every 10 seconds to save your exact timestamp
                setInterval(() => {
            ...
            try:
                ...
                async def update_progress(request: Request):
                    try:
                        ...
                        # Silently drop the ping if the browser sends an empty payload
                        pass
                    return {"status": "success"}

                # --- FIXED: Dynamic Chapter Extraction Route ---
