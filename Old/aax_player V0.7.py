...
class AAXManagerApp:
    ...
    def build_auth_components(self, parent):
        ...
        file_frame = tk.LabelFrame(parent, text="Active File Context", padx=10, pady=10)
        file_frame.pack(fill="x", padx=5, pady=5)

        tk.Button(file_frame, text="Load File Manually", command=self.load_file_prompt).pack(fill="x", pady=2)
        ...
        self.convert_btn.pack(fill="x", pady=2)
    ...
    def build_library_components(self, parent):
        ...
        cloud_tree_frame = tk.Frame(cloud_frame)
        cloud_tree_frame.pack(fill="both", expand=True, pady=5)

        cloud_scroll = ttk.Scrollbar(cloud_tree_frame)
        cloud_scroll.pack(side=tk.RIGHT, fill="y")

        self.cloud_tree = ttk.Treeview(cloud_tree_frame, columns=("Title", "Author", "Duration", "ASIN"), show="headings", yscrollcommand=cloud_scroll.set)
        cloud_scroll.config(command=self.cloud_tree.yview)

        for col in self.cloud_tree["columns"]:
            self.cloud_tree.heading(col, text=col, command=lambda _col=col: self.sort_treeview(self.cloud_tree, _col, False))
            
        self.cloud_tree.column("Title", width=300)
        ...
        cloud_btn_frame.pack(fill="x", pady=2)
        tk.Button(cloud_btn_frame, text="Refresh Cloud Library", command=self.fetch_cloud_library).pack(side=tk.LEFT, padx=5)
        tk.Button(cloud_btn_frame, text="Download Selected", command=self.download_title_prompt).pack(side=tk.LEFT, padx=5)
        tk.Button(cloud_btn_frame, text="Download All", command=self.download_all_prompt).pack(side=tk.LEFT, padx=5)
        ...
        local_tree_frame.pack(fill="both", expand=True, pady=5)

        local_scroll = ttk.Scrollbar(local_tree_frame)
        local_scroll.pack(side=tk.RIGHT, fill="y")

        self.local_tree = ttk.Treeview(local_tree_frame, columns=("Title", "Format", "Path"), show="headings", yscrollcommand=local_scroll.set)
        local_scroll.config(command=self.local_tree.yview)

        for col in self.local_tree["columns"]:
            self.local_tree.heading(col, text=col, command=lambda _col=col: self.sort_treeview(self.local_tree, _col, False))
            
        self.local_tree.column("Title", width=300)
        self.local_tree.column("Format", width=70)
        self.local_tree.column("Path", width=250)
        self.local_tree.pack(side=tk.LEFT, fill="both", expand=True)

        btn_frame = tk.Frame(local_frame)
        btn_frame.pack(fill="x", pady=5)
        tk.Button(btn_frame, text="Add File to Local Library", command=self.add_local_file).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame, text="Remove from List", command=self.remove_local_file).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame, text="Send to Player", command=self.send_to_player).pack(side=tk.RIGHT, padx=5)
    ...
    def build_player_components(self, parent):
        ...
        tk.Button(controls_frame, text="Play", width=8, command=self.play_chapter).pack(side=tk.LEFT, padx=2)
    ...
    def pause_audio(self):
        if self.is_playing and self.player_process:
            ...
            # Roll back 1.5 seconds on pause
