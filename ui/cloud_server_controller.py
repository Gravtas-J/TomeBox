...
class CloudServerController:
    ...
    def fetch_cloud_library(self):
        ...
        if not self.app.api_client.auth:
            self.app.logger.info(
                "DEBUG: fetch_cloud_library aborted - self.app.api_client.auth is missing or None."
            )
            self.app.action_router.event_bus.publish(
                "ui.show_warning",
                title="Not Logged In",
                message="Please login via the Settings tab first.",
            )
            return

        self.app.logger.info(
            "DEBUG: self.app.api_client.auth verified. Launching fetch_library_worker thread..."
        )
    ...
    def fetch_library_worker(self):
        try:
            ...
            self.app.logger.info(
                f"Successfully retrieved {len(self.app.library_manager.cloud_items)} library items."
            )
            ...
            self.app.metadata_manager.sync_missing_covers(
                on_complete_cb=lambda: self.app.root.after(
                    0,
                    lambda: (
            ...
            if (
                ...
                self.app.action_router.event_bus.publish(
                    ...
                    message="Could not connect to Audible servers. Check your internet connection.",
                )
            elif (
                "401" in err_str
                or "unauthorized" in err_str
                or "not authenticated" in err_str
            ):
                self.app.logger.error(f"Audible API rejected the request: {e}")
                self.app.action_router.event_bus.publish(
                    "ui.show_error",
                    title="Audible API Error",
                    message="Your session may have expired. Please log in again via Settings.",
                )
            else:
                self.app.logger.error(
                    f"Unhandled exception in library worker: {e}\n{traceback.format_exc()}"
                )
                self.app.action_router.event_bus.publish(
                    "ui.show_error",
                    title="Library Error",
                    message="An unexpected error occurred while fetching your library.",
                )
        finally:
    ...
    def run_background_sync(self):
        self.app.thread_pool.submit(
            ...
            self.app.logger,
            lambda msg: self.app.root.after(
                0, lambda: self.app.ui_state.dl_status.set(msg)
            ),
            lambda: self.app.root.after(
    ...
    def toggle_web_server(self):
        def on_started():
            ...
            self.app.root.after(
                0,
                lambda: self.app.file_menu.entryconfigure(
                    "Enable Web Server", label="Disable Web Server"
                ),
            )
            self.app.root.after(
                0,
                lambda: self.app.file_menu.entryconfigure(
                    "Show Pairing Info", state=tk.NORMAL
                ),
            )
        ...
        def on_stopped():
            ...
            self.app.root.after(
                0,
                lambda: self.app.file_menu.entryconfigure(
                    "Disable Web Server", label="Enable Web Server"
                ),
            )
            self.app.root.after(
                0,
                lambda: self.app.file_menu.entryconfigure(
                    "Show Pairing Info", state=tk.DISABLED
                ),
            )
            ...
            self.app.root.after(
                ...
                lambda: messagebox.showinfo(
                    "Server Stopped", "The companion server has been safely disabled."
                ),
            )
    ...
    def add_firewall_rule_prompt(self):
        if os.name != "nt":
            messagebox.showinfo(
                "Not Applicable", "Firewall management is only automated on Windows."
            )
            return
        ...
        if self.app.system_manager._is_firewall_rule_installed():
            messagebox.showinfo(
                "Already Installed",
                "The 'TomeBox Web Server' firewall rule is already active on your system.",
            )
            return
        ...
        if messagebox.askyesno(
            ...
            if success:
                ...
                messagebox.showerror(
                    "Action Failed",
                    "Failed to add the firewall rule. You may have declined the admin prompt.",
                )
    ...
    def remove_firewall_rule_prompt(self):
        ...
        if messagebox.askyesno(
            ...
            if success:
                ...
                messagebox.showerror(
                    "Action Failed",
                    "Failed to remove the firewall rule. You may have declined the admin prompt, or the rule did not exist.",
                )
