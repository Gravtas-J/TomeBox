...
class ImportSession:
            ...
            for task in valid_pending:
                ...
                if is_folder:
                    self.app.library_manager.import_folder(
                        folder_path=path, converter=self.app.converter, active_profile=self.app.active_profile,
                        on_status_cb=lambda msg: self.app.root.after(0, self.app.ui_state.dl_status.set, msg),
                        on_complete_cb=lambda c, t=0, p=path: self.app.action_router.on_import_finished(p, c, t),
                        logger=self.app.logger, on_progress_cb=lambda pct: self.app.root.after(0, lambda: self.app.ui_state.dl_progress.set(pct))
                    )
                else:
                    self.app.library_manager.import_files(
                        file_paths=[path], converter=self.app.converter, active_profile=self.app.active_profile,
                        on_status_cb=lambda msg: self.app.root.after(0, self.app.ui_state.dl_status.set, msg),
                        on_complete_cb=lambda c, t=0, p=path: self.app.action_router.on_import_finished(p, c, t),
    ...
    def on_file_drop(self, event):
        ...
        for path in dropped_paths:
            ...
            if os.path.isdir(path):
                self.app.system_manager.add_pending_import(self.app.db.data_dir, path, True)
                ...
                self.app.library_manager.import_folder(
                    ...
                    on_status_cb=lambda msg, tid=task_id: self.app.action_router.on_import_status(tid, msg),
                    on_complete_cb=lambda c, t=0, p=path, tid=task_id: self.app.action_router.on_import_finished(p, c, t, tid),
                    logger=self.app.logger, 
                    on_progress_cb=lambda pct, tid=task_id: self.app.action_router.on_import_progress(tid, pct),
                    task_id=task_id,
                ...
                if ext in [".aax", ".aaxc", ".m4b", ".mp3"]:
                    self.app.system_manager.add_pending_import(self.app.db.data_dir, path, False)
                    self.toggle_queue_drawer(True)
    ...
    def add_local_file(self):
        ...
        self.app.system_manager.add_pending_import(self.app.db.data_dir, filepath, False)
        ...
        self.app.library_manager.import_files(
            file_paths=[filepath], converter=self.app.converter, active_profile=self.app.active_profile,
            on_status_cb=lambda msg, tid=task_id: self.app.action_router.on_import_status(tid, msg),
            on_complete_cb=lambda c, t=0, p=filepath, tid=task_id: self.app.action_router.on_import_finished(p, c, t, tid),
            logger=self.app.logger,
            task_id=task_id
        )
    ...
    def import_folder(self):
        ...
        try:
            self.app.library_manager.import_folder(
                ...
                active_profile=self.app.active_profile,
                on_status_cb=lambda msg, tid=task_id: self.app.action_router.on_import_status(tid, msg),
                on_complete_cb=lambda c, t=0, p=folder, tid=task_id: self.app.action_router.on_import_finished(p, c, t, tid),
                logger=self.app.logger, 
                on_progress_cb=lambda pct, tid=task_id: self.app.action_router.on_import_progress(tid, pct),
                task_id=task_id,
                import_mode=import_mode,
