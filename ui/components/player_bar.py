...
class PlayerBarView:
    ...
    def build_ui(self):
        ...
        self.btn_compact = ttk.Button(
            ...
            text="▼ Compact",
            width=10,
            command=self.callbacks.get("toggle_compact"),
            state=tk.DISABLED,
        )
        ...
        self.progress_bar = ttk.Progressbar(
            self.play_frame, variable=self.ui_state.playback_progress, maximum=100
        )
        ...
        self.btn_prev = ttk.Button(
            self.playback_btns_frame,
            text="<< Prev Chapter",
            width=14,
            command=self.playback_presenter.prev_chapter,
        )
        self.btn_prev.pack(side=tk.LEFT, padx=2)

        self.btn_m30 = ttk.Button(
            self.playback_btns_frame,
            text="-30s",
            width=5,
            command=lambda: self.playback_presenter.seek_audio(-30),
        )
        ...
        self.btn_play = ttk.Button(
            ...
            text="Play",
            width=8,
            command=self.playback_presenter.master_play,
        )
        ...
        self.btn_pause = ttk.Button(
            ...
            text="Pause",
            width=8,
            command=self.playback_presenter.pause_audio,
        )
        self.btn_pause.pack(side=tk.LEFT, padx=2)

        self.btn_p30 = ttk.Button(
            self.playback_btns_frame,
            text="+30s",
            width=5,
            command=lambda: self.playback_presenter.seek_audio(30),
        ...
        self.btn_next = ttk.Button(
            ...
            text="Next Chapter >>",
            width=14,
            command=self.playback_presenter.next_chapter,
        )
        ...
        ttk.Button(
            ...
            text="🔖 Bookmark",
            width=12,
            command=self.bookmarks_presenter.add_bookmark,
        ).pack(side=tk.LEFT, padx=(10, 2))
        ttk.Button(
            self.extras_frame,
            text="📑 Chapters",
            command=self.callbacks.get("open_chapter"),
        ...
        speed_menu = ttk.Combobox(
            self.extras_frame,
            textvariable=self.ui_state.playback_speed,
        ...
        self.vol_slider = ttk.Scale(
            ...
            variable=self.ui_state.volume,
            command=self.playback_presenter.on_volume_change,
            length=80,
        )
        ...
        self.timer_btn = ttk.Button(
            timer_frame,
            text="Sleep: Off",
            command=self.callbacks.get("open_sleep"),
            width=16,
        )
        self.timer_btn.pack(side=tk.LEFT)

        ttk.Label(
            timer_frame, textvariable=self.ui_state.timer_countdown, width=5
        ).pack(side=tk.LEFT)
        ...
        ttk.Checkbutton(
            self.filters_frame,
            text="Voice Boost (Compressor)",
            variable=self.ui_state.voice_boost,
            command=self.callbacks.get("on_filter_change"),
        ).pack(side=tk.LEFT, padx=5)
        ttk.Checkbutton(
            self.filters_frame,
            text="Skip Silence",
            variable=self.ui_state.skip_silence,
            command=self.callbacks.get("on_filter_change"),
        ).pack(side=tk.LEFT, padx=5)
