...
class PlaybackPresenter:
    ...
    def on_playback_tick(self, current_time, total_time, real_time_delta):
        def update_ui():
            ...
            if self.app.session_listen_buffer >= 60.0:
                self.app.stats_manager.add_stat(
                    "seconds_listened", self.app.session_listen_buffer
                )
        ...
        try:
            ...
            if self.app.playback.chapters:
                start_offset = float(
                    self.app.playback.chapters[
                        self.app.playback.current_chapter_idx
                    ].get("start_time", 0)
                )
            ...
            if self.app.playback.seek_to_absolute(abs_position):
                ...
                if hasattr(self.app, "progress_bar") and self.app.playback.chapters:
                    ...
                    self.app.playback.chapter_duration = float(
                    ...
                    percent = (
                        (
                            self.app.playback.current_play_time
                            / self.app.playback.chapter_duration
                        )
                        * 100
                        if self.app.playback.chapter_duration > 0
                        else 0
    ...
    def cue_last_played(self):
        ...
        if (
            last_path
            and last_path in self.app.library_manager.local_library
    ...
    def load_specific_file(self, filepath):
        ...
        if hasattr(self.view, "player_cover_lbl"):
            ...
            if cover_path:
                try:
                    ...
                    self.view.player_cover_lbl.config(
                        image=thumb_photo, width=45, height=45
                    )
        ...
        if is_encrypted:
            ...
            if not success:
                ...
                self.app.action_router.event_bus.publish(
                    ...
                    message=f"Failed to process the file. Reason:\n\n{error_msg}",
                )
                self.app.file_path = ""
                return
        ...
        if cached_chapters:
            ...
            self.app.ui_state.dl_status.set(
                f"Extracting chapters: {os.path.basename(self.app.file_path)}"
            )
            self.app.root.update()
            ...
            self.app.library_manager.db.save_local_db(
                self.app.library_manager.local_library
            )

        self.app.ui_state.dl_status.set(
            f"Ready: {os.path.basename(self.app.file_path)}"
        )

        if not self.app.playback.chapters:
            self.app.logger.info(
                "No chapters found in file. Generating dummy master chapter."
            )
            duration_sec = local_data.get("duration_min", 0) * 60
            if duration_sec <= 0:
                try:
                    duration_sec = self.app.converter.get_duration(self.app.file_path)
                except Exception:
                    duration_sec = 86400
            self.app.playback.chapters = [
                {
        ...
        if abs_pos is not None:
            ...
            self.app.playback.current_play_time = max(
                0.0,
                abs_pos
                - float(self.app.playback.chapters[found_chap].get("start_time", 0)),
            )
        else:
        ...
        self.view.time_label.config(text=f"{curr_str} / {dur_str}")
        percent = (
            (self.app.playback.current_play_time / self.app.playback.chapter_duration)
            * 100
            if self.app.playback.chapter_duration > 0
            else 0
        )
    ...
    def verify_bytes(self, filepath):
        ...
        try:
            ...
            if result.returncode != 0:
                return (
                    False,
                    result.stderr if result.stderr else "FFmpeg rejected the file.",
                )
            return True, ""
    ...
    def master_play(self, event=None):
            ...
            if not selected:
                if self.app.file_path:
                    ...
                    self.app.action_router.event_bus.publish(
                        ...
                        message="Please select an audiobook to play.",
                    )
                return
        ...
        if "Downloaded" not in status:
            self.app.action_router.event_bus.publish(
                "ui.show_info",
                title="Cloud Only",
                message="This title has not been downloaded yet.",
            )
            return
        ...
        if not local_path or (not is_playlist and not os.path.exists(local_path)):
            self.app.action_router.event_bus.publish(
                "ui.show_error",
                title="File Error",
                message="The audio file could not be found on your disk.",
            )
            return

        # Load only if this isn't already the loaded file — load_specific_file
        # does chapter extraction, metadata fetch, cover thumbnail, etc.
        if local_path != self.app.file_path:
            self.load_specific_file(local_path)

        self.play_chapter()

    def play_chapter(self):
        if not self.app.file_path or not self.app.playback.chapters:
            return
    ...
    def resume_playback(self):
        ...
        if not self.app.playback.file_path or not os.path.isfile(
            self.app.playback.file_path
        ):
            self.app.logger.error(
                f"Playback Failed: Target is not a valid file -> {self.app.playback.file_path}"
            )
            self.stop_audio()
            return

        drm_flags = (
            self.app.api_client.get_drm_flags(
                self.app.file_path,
                local_data,
                self.app.active_profile,
                self.app.ui_state.auth_bytes.get().strip(),
                self.app.db.data_dir,
            )
            if self.app.file_path.endswith((".aax", ".aaxc"))
            else None
        )

        self.app.playback.set_speed(
            float(self.app.ui_state.playback_speed.get().replace("x", ""))
        )
    ...
    def on_progress_click(self, event):
        if (
            not hasattr(self.view, "progress_bar")
            or self.app.playback.chapter_duration <= 0
        ):
            return
        ...
        if self.app.playback.next_chapter():
            if self.app.sleep_mode == "chapters":
                ...
                if self.app.sleep_chapters_remaining <= 0:
                    ...
                    self.app.logger.info(
                        "Sleep timer (chapters) finished. Pausing playback."
                    )
                    ...
                    self.view.timer_btn.config(
                        text=f"Sleep: {self.app.sleep_chapters_remaining} ch"
                    )
    ...
    def update_info(self):
        if self.app.playback.chapters:
            title = (
                self.app.playback.chapters[self.app.playback.current_chapter_idx]
                .get("tags", {})
                .get("title", f"Chapter {self.app.playback.current_chapter_idx + 1}")
            )
    ...
    def set_sleep_timer(self, mode, value=0):
        ...
        if (
            hasattr(self.app, "sleep_menu_popup")
            and self.app.sleep_menu_popup.winfo_exists()
        ):
        ...
        if mode == "time":
            ...
            self.view.timer_btn.config(
                text=f"Sleep: {self.format_time(self.app.sleep_timer_seconds)}"
            )
    ...
    def on_sleep_timer_set(self, event=None):
        ...
        self.app.ui_state.timer_countdown.set(
            self.format_time(self.app.sleep_timer_seconds)
        )
        self.sleep_timer_tick()
