...
class ActionRouter:
    ...
    def _subscribe_events(self):
        # --- Download Events ---
        self.event_bus.subscribe("download.status", lambda **kw: self.on_dl_status(kw.get("asin"), kw.get("status"), kw.get("is_global", False)))
        self.event_bus.subscribe("download.progress", lambda **kw: self.on_dl_progress(kw.get("asin"), kw.get("percent"), kw.get("is_global", False)))
        self.event_bus.subscribe("download.complete", lambda **kw: self.on_dl_complete(kw.get("filepath"), kw.get("title"), kw.get("post_action")))
        self.event_bus.subscribe("download.batch_finish", lambda **kw: self.on_dl_batch_finish())

        # --- Metadata Events ---
        self.event_bus.subscribe("metadata.search_complete", lambda **kw: self.on_scrape_search_results(kw.get("filepath"), kw.get("products")))
        self.event_bus.subscribe("metadata.apply_complete", lambda **kw: self.on_scrape_apply_complete(kw.get("filepath"), kw.get("title")))
        self.event_bus.subscribe("metadata.display_ready", lambda **kw: self.on_display_metadata_ready(kw.get("filepath"), kw.get("cover_path"), kw.get("authors"), kw.get("msg")))
        self.event_bus.subscribe("metadata.error", lambda **kw: self.on_scrape_error(kw.get("error_msg")))

        # --- Conversion Events ---
        self.event_bus.subscribe("conversion.status", lambda **kw: self.app.root.after(0, self.update_global_status, kw.get("status")))
    ...
    def on_import_complete(self, added_count, total_found=0):
        def update():
            ...
            if added_count > 0:
                self.app.ui_state.dl_status.set(f"Successfully imported {added_count} files.")
            elif total_found > 0:
    ...
    def on_task_error(self, filepath, action_type, error_msg):
        def update():
            self.app.failed_tasks.append({"path": filepath, "action": action_type, "error": error_msg})
            self.app.ui_state.error_btn.set(f"Errors ({len(self.app.failed_tasks)})")
            self.app.error_btn.config(state=tk.NORMAL)
            self.app.ui_state.dl_status.set(f"Task failed: {os.path.basename(filepath)}")
        ...
        def update():
            ...
            if cover_path and os.path.exists(cover_path):
                try:
                    ...
                    self.app.cover_label.bind("<Button-1>", lambda e, a=asin, t=title, p=cover_path: open_cover_modal(self.app, a, t, explicit_path=p))
                except Exception:
