...
class BookmarksPresenter:
    ...
    def add_bookmark(self):
        ...
        if self.app.playback.chapters:
            abs_time += float(
                self.app.playback.chapters[chapter_idx].get("start_time", 0)
            )

        note = simpledialog.askstring(
            "Add Bookmark",
            f"Add a note for {self.app.playback_presenter.format_time(current_time)}:",
        )
    ...
    def refresh_bookmarks_ui(self):
        if not hasattr(self.app, "bm_tree"):
            return
        ...
        for idx, bm in enumerate(bookmarks):
            ...
            if (
                target_path == self.app.file_path
                and self.app.playback.chapters
                and chap_idx < len(self.app.playback.chapters)
            ):
                chap_title = (
                    self.app.playback.chapters[chap_idx]
                    .get("tags", {})
                    .get("title", chap_title)
                )

            t_str = self.app.playback_presenter.format_time(bm.get("time", 0))
            display_time = f"{chap_title} - {t_str}"

            self.app.bm_tree.insert(
                "", "end", iid=str(idx), values=(display_time, bm.get("note", ""))
            )
    ...
    def jump_to_bookmark(self, event=None):
        ...
        bookmarks = self.app.library_manager.local_library.get(target_path, {}).get(
            "bookmarks", []
        )
