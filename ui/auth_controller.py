...
class AuthController:
    ...
    def auto_load_auth(self):
        ...
        if self.app.api_client.load_auth_from_file(self.app.auth_save_path):
            ...
            self.app.logger.info(
                f"Session loaded automatically. Activation Bytes: {activation_bytes}"
            )
    ...
    def load_auth_file_prompt(self):
        filepath = filedialog.askopenfilename(
            filetypes=[("JSON Auth File", "*.json")], title="Select Audible Auth File"
        )
        if not filepath:
            return
        ...
        try:
            if self.app.api_client.load_auth_from_file(filepath):
                ...
                messagebox.showinfo(
                    "Success", "Auth file loaded! You can now fetch your library."
                )
                self.app.cloud_server_controller.fetch_cloud_library()
        except Exception:
    ...
    def start_browser_login_thread(self):
        if (
            hasattr(self.app, "browser_login_btn")
            and self.app.browser_login_btn
            and self.app.browser_login_btn.winfo_exists()
        ):
            self.app.browser_login_btn.config(text="Connecting...", state=tk.DISABLED)
        self.app.thread_pool.submit(
            self.browser_login_worker, self.app.ui_state.locale.get()
        )
    ...
    def browser_login_worker(self, locale):
        ...
        def custom_login_callback(login_url):
            ...
            def ask_user_for_url():
                ...
                res = simpledialog.askstring(
                    "Audible Login Authorization", msg, parent=self.app.root
                )
                result[0] = res
                event.set()
        ...
        try:
            self.app.logger.info(
                "Waiting for user to complete browser login and paste URL..."
            )
            if self.app.api_client.login_with_browser(locale, custom_login_callback):
                activation_bytes = self.app.api_client.get_activation_bytes()

                self.app.root.after(
                    0, self.app.ui_state.auth_bytes.set, activation_bytes
                )
                self.app.logger.info(f"Activation Bytes Received: {activation_bytes}")

                self.app.api_client.save_auth_to_file(self.app.auth_save_path)
                self.app.logger.info(
                    f"Session saved locally to {self.app.auth_save_path}"
                )

                self.app.root.after(
                    0, lambda: messagebox.showinfo("Success", "Connected to Audible!")
                )
                self.app.ui_state.filter.set("All")
                self.app.ui_state.shelf_filter.set("All Shelves")
                self.app.ui_state.search.set("")
                self.app.root.after(
                    0, self.app.cloud_server_controller.fetch_cloud_library
                )

        except Exception as e:  # Make sure it says 'as e'
            error_msg = str(e)
            self.app.logger.error("ERROR DURING LOGIN:")
            self.app.logger.error(traceback.format_exc())
            # Pass error_msg into the lambda so it doesn't lose scope
            self.app.root.after(0, lambda msg=error_msg: messagebox.showerror("Login Failed", msg))
            ...
            def restore_btn():
                if (
                    ...
                    self.app.browser_login_btn.config(
                        text="Login via Browser", state=tk.NORMAL
                    )
    ...
    def switch_profile(self, event=None):
        ...
        self.app.cloud_cache_path = self.app.db.get_cloud_cache_path(
        ...
        self.app.library_manager.cloud_items = (
            self.app.library_manager.load_cloud_cache(self.app.active_profile)
        )
