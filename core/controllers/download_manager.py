...
class DownloadManager:
    ...
    def pause_all(self):
        with self.queue_lock:
            self.is_paused = True
            self.event_bus.publish("download.status", asin="global", status="Queue Paused", is_global=True)

    def resume_all(self):
        with self.queue_lock:
            self.is_paused = False
            self.event_bus.publish("download.status", asin="global", status="Queue Active", is_global=True)
    ...
    def pause_download(self, asin):
        with self.queue_lock:
            if any(t["asin"] == asin for t in self.queue):
                ...
                self.event_bus.publish("download.status", asin=asin, status="Paused", is_global=False)

    def resume_download(self, asin):
        with self.queue_lock:
            if asin in self.paused_items:
                self.paused_items.remove(asin)
                self.event_bus.publish("download.status", asin=asin, status="Queued", is_global=False)
